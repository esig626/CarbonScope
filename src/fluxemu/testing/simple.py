...
class SimpleBinaryLawPair:
    ...
    Bruno-bound evaluation must call :func:`validate_bruno_assumptions` first.
...
@dataclass(frozen=True, slots=True, kw_only=True)
class BrunoOrderBound:
    """Order-specific lower bound on optimal Type-II error.
    ...
    logarithms are used. An order-specific result makes no claim about the
    global continuous-order envelope.
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.type_ii_lower_bound <= 1:
            raise InputValidationError("combined Type-II lower bound must be finite and lie in [0, 1]")
        if self.type_ii_lower_bound != max(self.reverse_lower_bound, self.forward_lower_bound):
            raise InputValidationError("Type-II lower bound must equal the maximum of both raw components")
    ...
    @property
    def global_envelope_evaluated(self) -> bool:
        return False
    ...
    def fingerprint(self) -> str:
        ...
        return _testing_digest(("bruno-order-bound-v1", self.pair.fingerprint,
