"""Integration gates for the complete native Stage 1 sampled pipeline."""
...
import fluxemu.emu.stationary as emu_module
...
from fluxemu.exceptions import AnalysisError, ForwardEMUError
...
def _alternate_path_science(
    ...
    """Return a nonlexically ordered optimal face with label-varying paths."""
    ...
    experiment = StationaryExperimentSemantics(
        ...
        (
            ObservationTarget(
                "I-observed",
                1,
                (ObservationPrecursor("I", (1,)),),
                (("source", "synthetic integration order gate"),),
            ),
        ),
    )
...
def test_composed_pipeline_reuses_flux_region_and_exact_sample_tuple(
    ...
    original_sampling = pipeline_module.sample_prepared_flux_states
    ...
    def record_sampling(prepared, count, **kwargs):
        calls["sampling"] += 1
        observed["sampling_prepared"] = prepared
        observed["sampling_count"] = count
        observed["sampling_fva"] = kwargs.get("fva")
        result = original_sampling(prepared, count, **kwargs)
        observed["sampling_result"] = result
        return result

    def record_compile_emu(canonical_model, stationary_experiment):
        calls["compile_emu"] += 1
        result = original_compile_emu(canonical_model, stationary_experiment)
        observed["emu_plan"] = result
        return result

    def record_evaluate_emu(plan, states, **kwargs):
        calls["evaluate_emu"] += 1
        observed["evaluated_plan"] = plan
        observed["evaluated_states"] = states
        observed["evaluate_kwargs"] = kwargs
        return original_evaluate_emu(plan, states, **kwargs)
    ...
    monkeypatch.setattr(
        pipeline_module, "sample_prepared_flux_states", record_sampling
    )
    ...
    assert observed["evaluated_plan"] is observed["emu_plan"]
    assert observed["evaluated_states"] is result.flux_sampling.states
    assert observed["evaluate_kwargs"] == {"mid_tolerance": 3e-10}
    assert observed["sampling_count"] == 4
    assert result.model_fingerprint == model_fingerprint(model)
    assert result.experiment_fingerprint == experiment_fingerprint(
        model, experiment
    )
...
def test_fraction_one_ensemble_replays_ordered_distinct_mids() -> None:
    ...
    second = run_native_stationary_ensemble_analysis(
    ...
    assert sampling.provenance.optimal_face
    ...
    assert sample_ids == tuple(f"sample_{index:04d}" for index in range(7))
    assert all(
    ...
    assert tuple(
        (item.sample_id, item.target_id) for item in predictions
    ) == tuple(
        (sample_id, target_id)
        for sample_id in sample_ids
        for target_id in ("O-mid", "I-observed")
    )
    assert len({item.fractions for item in predictions}) > 1
    for state, state_predictions in zip(
        sampling.states,
        (predictions[index : index + 2] for index in range(0, 14, 2)),
        strict=True,
    ):
        flux = dict(state.values)
        for prediction in state_predictions:
    ...
    assert tuple(item.sample_id for item in first.mid_ensemble.forward.values) == tuple(
        sample_id for sample_id in sample_ids for _ in range(4)
    )
    assert tuple(
...
def test_sampling_failure_stops_before_emu_work(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model, experiment = _alternate_path_science()
    calls = {"sampling": 0, "compile_emu": 0, "evaluate_emu": 0}
    ...
    monkeypatch.setattr(
        pipeline_module, "sample_prepared_flux_states", fail_sampling
    )
    ...
    assert calls == {"sampling": 1, "compile_emu": 0, "evaluate_emu": 0}


def test_emu_failure_propagates_without_resampling_or_filtering(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model, experiment = _alternate_path_science()
    calls = {"sampling": 0, "states": []}
    original_sampling = pipeline_module.sample_prepared_flux_states
    original_evaluate_state = emu_module._evaluate_state

    def record_sampling(prepared, count, **kwargs):
        calls["sampling"] += 1
        return original_sampling(prepared, count, **kwargs)

    def fail_second_state(plan, state, tolerance):
        calls["states"].append(state.sample_id)
        if state.sample_id == "sample_0001":
            raise ForwardEMUError("deliberate sampled-state EMU failure")
        return original_evaluate_state(plan, state, tolerance)

    monkeypatch.setattr(
        pipeline_module, "sample_prepared_flux_states", record_sampling
    )
    monkeypatch.setattr(emu_module, "_evaluate_state", fail_second_state)

    with pytest.raises(
        ForwardEMUError,
        match="flux state 'sample_0001': deliberate sampled-state EMU failure",
    ):
        run_native_stationary_ensemble(
            model,
            experiment,
    ...
    assert calls == {
        "sampling": 1,
        "states": ["sample_0000", "sample_0001"],
    }


@pytest.mark.parametrize("mutation", ["reverse", "drop", "append"])
def test_pipeline_rejects_mutated_mid_prediction_order_or_count(
    mutation: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model, experiment = _alternate_path_science()
    original_evaluate = pipeline_module.evaluate_stationary

    def mutate_predictions(plan, states, **kwargs):
        result = original_evaluate(plan, states, **kwargs)
        predictions = result.forward.predictions
        if mutation == "reverse":
            changed = tuple(reversed(predictions))
        elif mutation == "drop":
            changed = predictions[:-1]
        else:
            changed = predictions + predictions[:1]
        return replace(result, forward=replace(result.forward, predictions=changed))

    monkeypatch.setattr(
        pipeline_module, "evaluate_stationary", mutate_predictions
    )

    with pytest.raises(
        AnalysisError,
        match=(
            "native stationary MID ensemble does not preserve sampled-state "
            "and declared-target order"
        ),
    ):
        run_native_stationary_ensemble(
            model,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("sample_count", True, "sample_count must be a positive integer"),
        ...
        (
            "mid_tolerance",
            -1.0,
            "mid_tolerance must be a finite nonnegative number",
        ),
        (
            "mid_tolerance",
            float("nan"),
...
def test_ensemble_api_and_analysis_aliases_are_public() -> None:
    ...
    assert (
        fluxemu.run_native_stationary_ensemble_analysis
        is run_native_stationary_ensemble
    )
    assert run_native_stationary_ensemble_analysis is run_native_stationary_ensemble
    ...
    assert NativeStationaryEnsembleAnalysisResult is NativeStationaryEnsembleResult
    assert (
        fluxemu.analysis.NativeStationaryEnsembleAnalysisResult
        is NativeStationaryEnsembleResult
    )


def test_stage_b2_sampled_batch_produces_twelve_mids_per_state() -> None:
    ...
    assert tuple(item.target_id for item in predictions) == tuple(
        coverage.display_name
        for _ in sample_ids
        for coverage in __import__(
            "fluxemu.real_model", fromlist=["TARGET_COVERAGE"]
        ).TARGET_COVERAGE
    )
    assert all(min(item.fractions) >= -1e-10 for item in predictions)
    assert all(
        sum(item.fractions) == pytest.approx(1.0, abs=1e-10)
...
def test_public_ensemble_executes_with_optional_solver_stacks_blocked() -> None:
    script = r'''
import sys
for name in ("cobra", "optlang", "mfapy", "scipy"):
...
assert len(result.model_fingerprint) == len(result.experiment_fingerprint) == 64
assert all(
