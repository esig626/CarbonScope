...
FluxEMU includes a native fixed-flux transient EMU engine for explicit tracer-step experiments.
...
No clipping, simplex projection or hidden renormalisation is performed. Invalid/singular configurations fail explicitly.

## Terminal targets

For an unbalanced terminal target, FluxEMU reports the instantaneous flux-weighted production MID rather than inventing an unmodelled extracellular mixing pool.
