...
\title{Technical Note: p-values, likelihood ratios, and information-theoretic divergence in CarbonScope}
\author{CarbonScope technical notes}
...
CarbonScope now distinguishes four related but different objects:
\begin{enumerate}
...
CarbonScope's exact simple-vs-simple p-value, developed below, does not require this
...
H_1:v=v_1,
\end{equation}
CarbonScope maps each flux state through stationary EMU prediction and the declared
...
CarbonScope uses both Rényi directions to lower-bound the optimal Type-II error
...
CarbonScope uses divergences in two distinct settings that should not be conflated.
...
\section{Exact finite-sample implementation in CarbonScope}

For \texttt{likelihood\_ratio\_p\_value}, CarbonScope enumerates the complete
...
\section{Worked CarbonScope mixer example}
